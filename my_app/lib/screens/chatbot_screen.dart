                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text('Processing...', style: TextStyle(fontSize: 16.0)),
                      ],
                    ),
                  ),
                ],
 ...
 void _handleVoiceRecording() async {
  ...
  Future<void> _handleVoiceSubmit(String audioPath) async {
    ...
    try {
      ...
      setState(() {
        _messages.add(ChatMessage(
          text: "Sorry, I couldn't process your voice message. Please try again later.",
          isUserMessage: false,
        ));
        _isLoading = false;
      });
  ...
  try {
    ...
    if (streamedResponse.statusCode == 200) {
      ...
      final response = await http.Response.fromStream(streamedResponse);
      final errorData = jsonDecode(response.body);
      throw Exception(errorData['detail'] ?? 'Failed to get voice chat response');
    }
  } catch (e) {
